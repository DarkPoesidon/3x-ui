...
var defaultValueMap = map[string]string{
	...
	// On by default because it is what makes the subscription usable at all for
	// a whole family of clients. AnyTLS runs on sing-box and mihomo; mihomo
	// cannot read the base64 link list, so with this off it receives a blob it
	// rejects outright and the operator sees "the config will not load" with
